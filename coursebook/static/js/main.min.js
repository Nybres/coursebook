addEventListener('DOMContentLoaded', (event) => {
		...
		course.forEach((cour) => {
         ...
         <div class="offert-card">
            <div class="offert-card__image">
               <img width="400" height="200" loading="lazy" src="https://placehold.co/400x200?text=${cour.title}"
                  alt="" />
            </div>
	...
	// cart - creating object to checkout

	// document.getElementById('checkout-button').addEventListener('click', () => {
	// 	let orderName = document.getElementById('cart-order-name').value
	// 	let orderSurname = document.getElementById('cart-order-surname').value
	// 	let emailAddress = document.getElementById('cart-order-email').value
	// 	let phoneNumber = document.getElementById('cart-order-phone').value
	...
	// document.querySelector('.jsCreateCheckout').addEventListener('submit', function (event) {
	// 	event.preventDefault() // Zapobieganie domyślnej akcji formularza (przeładowanie strony)

	// 	// Znajdź wszystkie elementy formularza, które są przeznaczone dla uczestników
	// 	const participantForms = document.querySelectorAll('.mainCartHead__container__boxes__box-left-item-body-member')

	// 	// Tablica do przechowywania danych uczestników
	// 	const participantsData = []

	// 	participantForms.forEach(function (participantForm) {
	// 		event.preventDefault()
	// 		const inputs = participantForm.querySelectorAll('.form__group-input')
	// 		const participant = {}

	// 		inputs.forEach(function (input) {
	// 			participant[input.placeholder] = input.value // Dodajemy dane z pól do obiektu uczestnika
	// 		})

	// 		participantsData.push(participant) // Dodajemy obiekt uczestnika do tablicy
	// 	})

	// 	// Możesz teraz wykorzystać participantsData - tablicę obiektów z danymi uczestników
	...
	// const createOrderObjectToCheckout = () => {
	const orderCheckoutData = document.querySelector('.jsCreateCheckout')

	if (orderCheckoutData) {
		const cartOrderName = document.getElementById('cart-order-name')
		const cartOrderSurname = document.getElementById('cart-order-surname')
		const cartOrderemail = document.getElementById('cart-order-email')
		const cartOrderPhone = document.getElementById('cart-order-phone')

		// let purchasedCour = new PurchasedCourses(47, 2)

		function Purchaser(name, surname, email, phone) {
			this.name = name
		...
		let newObjectPurchaser = new Purchaser(
			cartOrderName.value,
			cartOrderSurname.value,
			cartOrderemail.value,
			cartOrderPhone.value
		)

		newObjectPurchaser.purchased_courses = [55, 11]

		console.log(newObjectPurchaser)
	...
	const orderQuantityAndType = (orders) => {
		orders.forEach( el => {
			orderId = el.id
