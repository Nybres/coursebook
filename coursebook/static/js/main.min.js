addEventListener('DOMContentLoaded', (event) => {
	...
	const provinceLabels = (item) => {
		...
		provinceLabel.innerHTML = `<li class="splide__slide offert__nav jsLabel">${item}</li>`
	}
			...
			if (response.ok) {
				response.json().then((data) => {
					...
					for (let i = 0; i < data.length; i++) {
						const provincesElements = data[i].province_name
						provinceItem = provincesElements

						if (provinceItem) {
							provinceLabels(provinceItem)
						}
					}
