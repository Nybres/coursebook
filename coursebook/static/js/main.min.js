addEventListener('DOMContentLoaded', (event) => {
	...
	// walidacja inputa w karcie produktu - trochę mi nie wyszła bo jak się wpisze liczbę dwucyfrową to można ją wpisać
	...
	if (quantityInputOfferCard) {
		...
		const setsValue = quantityInputOfferCard.getAttribute('data-quantity')

		quantityInputOfferCard.addEventListener('input', () => {
			const currentInputValue = quantityInputOfferCard.value

			// if (previousValue + currentInputValue > setsValue) {
			// 	quantityInputOfferCard.value = setsValue - 1
			// }

			if (/^\d*$/.test(currentInputValue) && currentInputValue > 0 && currentInputValue <= setsValue) {
				previousValue = currentInputValue
			} else if (currentInputValue > setsValue) {
				// tutaj próbowałem dodatkowo zabezpieczyć aby przy wpisaniu np "25" z klawiatury, aby nie przyjeło tego 25 i żeby wpisało liczbę
...
// 	const form = document.querySelector('jsValidForm')
// 	const reqInputs = form.querySelectorAll(".js-req");
// 	let error = 0;
// 	let arrayOfErrorToJump = [];


// 	reqInputs.forEach((validInput) => {
// 		validInput.addEventListener('focusout', function () {
// 			 if (validInput.getAttribute('data-type') == 'text') {
// 				  checkLength(validInput, 3)
// 			 } else if (validInput.getAttribute('data-type') == 'email') {
// 				  checkEmail(validInput)
// 			 } else if (validInput.getAttribute('data-type') == 'phone') {
// 				  checkPhone(validInput)
// 			 } else if (validInput.getAttribute('data-type') == 'password') {
// 				  checkAgreement(validInput)
// 			 }
// 		})
//   })
