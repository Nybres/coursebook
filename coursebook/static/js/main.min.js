addEventListener('DOMContentLoaded', (event) => {
	...
	const addQuantityToForm = (form) => {
		const cartInputPlus = document.querySelector('.nav__cart-container-boxes-box-price-form-plus')
		const cartInputMinus = document.querySelector('.nav__cart-container-boxes-box-price-form-minus')
		let cartInputValue = document.getElementById('jsCartInput')

		
		
		// cartInputValue.addEventListener('keyup', (e) => {                        // niedokonczona walidacja formularza przy wpisywaniu z ręki - pozniej mozna usunac
		// 	let input = e.target
		// 	let inputValue = input.value
		// 	let quantityOfOrder = cartInputValue.getAttribute('data-max-qty')
		// 	const cleanedValue = inputValue.replace(/\D/g, '')
		// 	if (inputValue !== cleanedValue) {
		// 		cartInputValue.style.border = '1px solid red'
		// 	} else if (inputValue == cleanedValue && inputValue > 0 && inputValue <= quantityOfOrder) {
		// 		cartInputValue.style.border = '1px solid #333333'
	...
	const createMiniCart = (payload) => {
		...
		if (cartElement) {
            ...
            <p class="nav__cart-container-footer-sum-text-price"><span class="jsSumPrice">0,00</span> zł</p>
        		...
        		<a title="Poznaj oferte" href="#" class="btn btn--primary nav__cart-container-footer-btn">Do kasy
				...
				cartItems.forEach((cartItem) => {
					...
					<form class="jsQuantity" method="PATCH" action="${URL}/cart/increase-quantity/${cartItem.cartItemId}">
						<div class="nav__cart-container-boxes-box-price-form">
							...
							<input id="jsCartInput" data-max-qty="${cartItem.course.seats}" name="quantity" type="text" value="${cartItem.quantity}" class="nav__cart-container-boxes-box-price-form-input" readonly>
